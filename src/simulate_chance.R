simulate.staircase.below<- function(target)
{
  ...
  for(j in 1:50)
  {
    ...
    {
      rowIndex<-rowIndex+1
      rowIndex<-min(numSteps, rowIndex)
      jnd[j+1]<- steps[rowIndex,3]
    }
    if(answer == 0)
    {
      rowIndex<-rowIndex-3
      rowIndex<-max(1, rowIndex)
      jnd[j+1]<- steps[rowIndex,3]
    }
    if(j>23)
    {
...
#simulate chance
rm(chanceBoundary)
rm(chanceByCondition)
targets<-c(0.8,0.7,0.6,0.5,0.4,0.3)
chanceByCondition<- data.frame(matrix(ncol= 3, nrow = 12))
colnames(chanceByCondition) <- c("target", "approach", "jnd24")
...
for(i in 1:length(targets))
{
  print(targets[i])
  #above
  chanceBoundary <-  data.frame(matrix(ncol= 3, nrow = 100))
  for(j in 1:nrow(chanceBoundary))
  {
    chanceBoundary[j,]<-simulate.staircase.above(targets[i])
  }
  rowIndex<- i*2-1
  chanceByCondition[rowIndex,1]<-targets[i]
  chanceByCondition[rowIndex,2]<-"above"
  chanceByCondition[rowIndex,3]<-mean(chanceBoundary[,1])
  
  #below
  chanceBoundary <-  data.frame(matrix(ncol= 3, nrow = 100))
  for(j in 1:nrow(chanceBoundary))
  {
    chanceBoundary[j,]<-simulate.staircase.below(targets[i])
  }
  rowIndex<- i*2
  chanceByCondition[rowIndex,1]<-targets[i]
  chanceByCondition[rowIndex,2]<-"below"
  chanceByCondition[rowIndex,3]<-mean(chanceBoundary[,1])
}
simulate.staircase.below(0.8)
...
{
  #above
  chance_boundary <-  data.frame(matrix(ncol= 3, nrow = 1000))
  for(j in 1:nrow(chance_boundary))
  {
    chance_boundary[j,]<-simulate.staircase.above(targets[i])
  }
  rowIndex<- i*2-1
  chance_by_condition[rowIndex,1]<-targets[i]
  chance_by_condition[rowIndex,2]<-"above"
  chance_by_condition[rowIndex,3]<-mean(chance_boundary[,1])
  ...
  rowIndex<- i*2
  chance_by_condition[rowIndex,1]<-targets[i]
  chance_by_condition[rowIndex,2]<-"below"
  chance_by_condition[rowIndex,3]<-mean(chance_boundary[,1])
}
